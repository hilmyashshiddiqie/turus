...
if (isset($_POST['p'])) {
    ...
    //---------------------------------------------ADMIN USER------------------------------------------------------------------------
    //MENAMPILKAN USER DI MANAJEMEN USER
    elseif ($_POST['p'] == "read") {
        if (isset($_POST['cari'])) {
            $id = $_POST['cari'];
            // $sql = "SELECT * FROM data_warga LEFT JOIN data_tagihan 
            // ON data_warga.id=data_tagihan.id_warga WHERE data_warga.id = $id";
            ...
            // $id = $_POST['cari'];
            // $sql = "SELECT * FROM data_warga JOIN data_tagihan 
            // ON data_warga.id=data_tagihan.id_warga";
            $sql = "SELECT * FROM data_warga";
            $query = mysqli_query($conn, $sql);
            $data = [];
    ...
    elseif ($_POST['p'] == "get_year_pemakaian_byuser") {
        ...
        $sql = "SELECT t.lunas, w.id as w_id, t.pemakaian as pemakaian, t.id as t_id, s.id as s_id, w.nama, t.stand_awal as t_awal, t.stand_akhir as t_akhir, s.stand_awal as s_awal, s.stand_akhir as s_akhir, s.tanggal, t.tagihan
                    ...
                    WHERE w.id = $idLog AND  tanggal BETWEEN '$year-01-01' AND '$year-12-31'
            ";
    ...
    //MENGATUR SWITCH ON/OFF
    elseif ($_POST['p'] == "switch") {
        // echo "masuk sini ";
        // if (isset($_POST['id_warga']) && isset($_POST['status_sistem'])) {
        $id = $_POST['id'];
        $status_sistem = $_POST['status_sistem'];

        $sql_warga = "SELECT node_id FROM data_warga WHERE id=" . $id;
        $data = mysqli_query($conn, $sql_warga);
        $data_warga = mysqli_fetch_assoc($data);
        $node_id = $data_warga["node_id"];
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => '{
        "m2m:cin": {
            "con": "{\\"type\\":\\"downlink\\", \\"data\\":\\"relay_' . $node_id . '_' . ($status_sistem == "ON" ? 1 : 0) . '\\"}"
        }
        }
        ',
        ...
        mysqli_query($conn, "UPDATE data_warga SET status_sistem='$status_sistem' WHERE id = $id");
        // echo "UPDATE data_tagihan SET status_sistem=$status_sistem WHERE id_warga = $id";
        // echo "UPDATE data_tagihan SET status_sistem=$status_sistem WHERE id_warga = $id";
    ...
    //--------------------------------------------------------------------*HARA------------------------------------------------------
    //DASHBOARD BENDAHARA
    //MENAMPILKAN USER YANG BELUM BAYAR (tabel)
    elseif ($_POST['p'] == "read_blm_byr") {
        if (isset($_POST['cari'])) {
            $id = $_POST['cari'];
            $sql = "SELECT * FROM data_warga LEFT JOIN data_tagihan 
            ON data_warga.id=data_tagihan.id_warga WHERE lunas='belum'";
    ...
    //SEARCH TABEL USER BLM BAYAR
    elseif ($_POST['p'] == "search2") {
        if (isset($_POST['key'])) {
            if ($_POST['key'] == null) {
                ...
                $sql_halaman = "SELECT count(*) as total FROM data_warga LEFT JOIN data_tagihan ON data_warga.id=data_tagihan.id_warga WHERE lunas='belum'";
                $query_halaman = mysqli_query($conn, $sql_halaman);
                $total_halaman = mysqli_fetch_assoc($query_halaman);
                ...
                if ($_POST['page'] == 0) {
                    $sql = "SELECT * FROM data_warga LEFT JOIN data_tagihan 
                    ON data_warga.id=data_tagihan.id_warga WHERE (nama LIKE '%$key%' OR alamat LIKE '%$key%' OR tipe LIKE '%$key%') ";
                } else {
                    $start_from = $_POST['page'];
                    $sql = "SELECT * FROM data_warga LEFT JOIN data_tagihan 
                    ON data_warga.id=data_tagihan.id_warga WHERE (nama LIKE '%$key%' OR alamat LIKE '%$key%' OR tipe LIKE '%$key%') ";
                ...
                $sql_halaman = "SELECT count(*) as total FROM data_warga LEFT JOIN data_tagihan 
                ON data_warga.id=data_tagihan.id_warga WHERE (nama LIKE '%$key%' OR alamat LIKE '%$key%' OR tipe LIKE '%$key%')";
    ...
    //MENAMPILKAN TABEL MENEJEMEN PEMBAYARAN
    elseif ($_POST['p'] == "manajemenair") {
        if (isset($_POST['look'])) {
            ...
            $year = date('Y', strtotime($_POST['tahuntag']));
            $id = $_POST["id_warga"];
            // $sql = "SELECT * FROM data_tagihan LEFT JOIN data_stand ON data_stand.id = data_tagihan.id_stand WHERE data_tagihan.id_warga = $id AND YEAR(data_stand.tanggal) = $year";
        ...
        if (isset($_POST['alamat'])) {
            ...
            if (empty($data)) {
                ...
                for ($i = 0; $i < count($data_id); $i++) {
                    ...
                    $sql = "INSERT into data_stand(tanggal,id_warga,stand_awal,stand_akhir,foto_bukti) VALUES ('$tanggal',$data_id[$i],$stand_awal, 0)";
                    mysqli_query($conn, $sql);
                }
    ...
    elseif ($_POST['p'] == "savepetugas") {
        ...
        if (isset($_POST["date_start"]) && isset($_POST["date_end"])) {
            $total = "SELECT * FROM (SELECT IFNULL(SUM(pemakaian),0) as pemakaian, month(data_stand.tanggal) as month, year(data_stand.tanggal) as year, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE (tanggal BETWEEN '" . $_POST["date_start"] . "' AND '" . $_POST["date_end"] . "');";
        }
        ...
        if (isset($_POST["date_start"]) && isset($_POST["date_end"])) {
            // $total = "SELECT * FROM (SELECT IFNULL(SUM(tagihan),0) as tagihan, month(data_stand.tanggal) as month, year(data_stand.tanggal) as year FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as tagihan WHERE (tanggal BETWEEN '" . $_POST["date_start"] . "' AND '" . $_POST["date_end"] . "');";
            $total = "SELECT * FROM (SELECT IFNULL(SUM(tagihan),0) as tagihan, month(data_stand.tanggal) as month, year(data_stand.tanggal) as year,data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal) as tagihan WHERE (tanggal BETWEEN '" . $_POST["date_start"] . "' AND '" . $_POST["date_end"] . "');";
        }
    ...
    // else if ($_POST['p'] == 'gaugeair') {
    //     $gaugeair = "SELECT VolPem FROM `node` ORDER BY `id` DESC LIMIT 1";
    //     $query  = mysqli_query($conn, $gaugeair);
    //     $data = mysqli_fetch_assoc($query);
    //     // while ($row = mysqli_fetch_assoc($query)) {
    //     //     $data[] = $row;
    ...
    //     echo json_encode($data);
    // } 
    else if ($_POST['p'] == 'gaugeair1') {
        $gaugeair = "SELECT VolPem FROM node WHERE NodeNum='1' ORDER BY id DESC LIMIT 1";
        $query  = mysqli_query($conn, $gaugeair);
        $data = mysqli_fetch_assoc($query);
        // while ($row = mysqli_fetch_assoc($query)) {
        //     $data[] = $row;
        // }
        echo json_encode($data);
    } else if ($_POST['p'] == 'gaugeair2') {
        $gaugeair = "SELECT VolPem FROM node WHERE NodeNum='2' ORDER BY id DESC LIMIT 1";
        $query  = mysqli_query($conn, $gaugeair);
        $data = mysqli_fetch_assoc($query);
        // while ($row = mysqli_fetch_assoc($query)) {
        //     $data[] = $row;
        ...
        // }
        echo json_encode($data);
        //MENAMPILKN DATA KONF BAYAR
    } else if ($_POST['p'] == 'volumtandon') {
        $volumtandon = "SELECT VolTan FROM `core` ORDER BY id DESC LIMIT 1";
        $query  = mysqli_query($conn, $volumtandon);
        $data = mysqli_fetch_assoc($query);
        // while ($row = mysqli_fetch_assoc($query)) {
        //     $data[] = $row;
        // }
    ...
    } else if ($_POST['p'] == 'kekeruhan') {
        $kekeruhan = "SELECT Tur FROM `core` ORDER BY id DESC LIMIT 1";
        $query  = mysqli_query($conn, $kekeruhan);
        $data = mysqli_fetch_assoc($query);
        // while ($row = mysqli_fetch_assoc($query)) {
        //     $data[] = $row;
        // }
    ...
    if ($tgl1 == $tgl2) {
        $rows1['name'] = 'jam';
        $querywaktu = mysqli_query($conn, "SELECT HOUR(tanggal) as jam FROM core WHERE tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM core WHERE tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
    } else if (empty($tgl2)) {
        $rows1['name'] = 'hari';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%H:%i') as harian FROM (SELECT * FROM core WHERE tanggal >='$tgl1' ORDER BY id DESC LIMIT 20) Var1 ORDER BY id ASC");
        $querynilai = mysqli_query($conn, "SELECT $identifier FROM core WHERE tanggal >='$tgl1' ORDER BY id DESC LIMIT 20");
    } else {
        $rows1['name'] = 'tanggal';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%Y-%m-%d') as jaraktanggal FROM core WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM core WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
    }
    while ($resultime = mysqli_fetch_array($querywaktu)) {
        if ($tgl1 == $tgl2) {
            $rows1['data'][] = $resultime['jam'];
        } else if (empty($tgl2)) {
            $rows1['data'][] = $resultime['harian'];
        } else {
            $rows1['data'][] = $resultime['jaraktanggal'];
        }
    }

    $rows2 = array();
    $rows2['name'] = $rowsname;
    while ($resultnilai = mysqli_fetch_array($querynilai)) {
        $rows2['data'][] = $resultnilai[$rowsname];
    }
    ...
    if ($tgl1 == $tgl2) {
        ...
        $rows1['name'] = 'hari';
        $querywaktu = mysqli_query($conn, "SELECT * FROM (SELECT month(data_stand.tanggal) as month, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE tanggal = '$tgl1'");
        $querynilai = mysqli_query($conn, "SELECT * FROM (SELECT IFNULL(SUM(pemakaian),0) as pemakaian, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE tanggal = '$tgl1'");
    } else {
        $rows1['name'] = 'tanggal';
        $querywaktu = mysqli_query($conn, "SELECT * FROM (SELECT month(data_stand.tanggal) as month, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2')");
        $querynilai = mysqli_query($conn, "SELECT * FROM (SELECT IFNULL(SUM(pemakaian),0) as pemakaian, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE (tanggal BETWEEN '$tgl1' AND '$tgl2')");
        // SELECT * FROM (SELECT IFNULL(SUM(pemakaian),0) as pemakaian, month(data_stand.tanggal) as month, year(data_stand.tanggal) as year, data_stand.tanggal FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC) as pemakaian WHERE (tanggal BETWEEN '" . $_POST["date_start"] . "' AND '" . $_POST["date_end"] . "')
        // SELECT IFNULL(SUM(pemakaian),0) as pemakaian, month(data_stand.tanggal) as month, year(data_stand.tanggal) as year FROM data_tagihan  RIGHT JOIN data_stand ON data_stand.id = data_tagihan.id_stand GROUP BY month(data_stand.tanggal), year(data_stand.tanggal) ORDER BY data_stand.tanggal DESC LIMIT 12
    }
    while ($resultime = mysqli_fetch_array($querywaktu)) {
        if ($tgl1 == $tgl2) {
            $rows1['data'][] = $resultime['jam'];
...
//date picket warga
    ...
    if ($tgl1 == $tgl2) {
        $rows1['name'] = 'jam';
        $querywaktu = mysqli_query($conn, "SELECT HOUR(tanggal) as jam FROM node WHERE NodeNum='1' and tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM node WHERE NodeNum='1' and tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
    } else if (empty($tgl2)) {
        $rows1['name'] = 'hari';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%H:%i') as harian FROM (SELECT * FROM node WHERE NodeNum='1' and tanggal >='$tgl1' ORDER BY id DESC LIMIT 20) Var1 ORDER BY id ASC");
        $querynilai = mysqli_query($conn, "SELECT $identifier FROM node WHERE NodeNum='1' and tanggal >='$tgl1' ORDER BY id DESC LIMIT 20");
    } else {
        $rows1['name'] = 'tanggal';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%Y-%m-%d') as jaraktanggal FROM node WHERE NodeNum='1' and (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM node WHERE NodeNum='1' and (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
    }
    while ($resultime = mysqli_fetch_array($querywaktu)) {
        if ($tgl1 == $tgl2) {
            $rows1['data'][] = $resultime['jam'];
    ...
    if ($tgl1 == $tgl2) {
        $rows1['name'] = 'jam';
        $querywaktu = mysqli_query($conn, "SELECT HOUR(tanggal) as jam FROM node WHERE NodeNum='2' and tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM node WHERE NodeNum='2' and tanggal >='$tgl1' GROUP BY HOUR(tanggal)");
    } else if (empty($tgl2)) {
        $rows1['name'] = 'hari';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%H:%i') as harian FROM (SELECT * FROM node WHERE NodeNum='2' and tanggal >='$tgl1' ORDER BY id DESC LIMIT 20) Var1 ORDER BY id ASC");
        $querynilai = mysqli_query($conn, "SELECT $identifier FROM node WHERE NodeNum='2' and tanggal >='$tgl1' ORDER BY id DESC LIMIT 20");
    } else {
        $rows1['name'] = 'tanggal';
        $querywaktu = mysqli_query($conn, "SELECT date_format(tanggal, '%Y-%m-%d') as jaraktanggal FROM node WHERE NodeNum='2' and (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
        $querynilai = mysqli_query($conn, "SELECT AVG($identifier) as $identifier FROM node WHERE NodeNum='2' and (tanggal BETWEEN '$tgl1' AND '$tgl2') GROUP BY tanggal");
    }
    while ($resultime = mysqli_fetch_array($querywaktu)) {
        if ($tgl1 == $tgl2) {
            $rows1['data'][] = $resultime['jam'];
